...
let PersonaController = class PersonaController {
    ...
    async identificarPersona(credenciales) {
        ...
        if (p) {
            ...
            throw new rest_1.HttpErrors[401]("Los datos ingresados son invalidos");
        }
    }
    async create(persona) {
        ...
        fetch(`${llaves_1.Llaves.urlServicioNotificaciones}/notificacion-email?correo_destino=${destino}&asunto=${asunto}&contenido=${contenido}`)
            .then((data) => {
            console.log(data);
        });
        return p;
